...
const ResultsPage = () => {
    ...
    const fetchJobs = useCallback(async (reset = false) => {
        ...
        setLoading(true);
        try {
            const currentPage = reset ? 0 : page; // Track page inside function
            const response = await fetch(
                `http://localhost:8080/api/jobs?title=${encodeURIComponent(jobTitle)}&page=${currentPage}&size=10`
            );
    ...
    // ✅ Filter jobs without extra re-fetches
